...
> Kyverno cleanup-controller always requires RBAC permissions for deleting resources (even pods). However, RBAC permissions vary by Kyverno installation method. In some testing clusters like `Minikube` and `Kind`, default permissions may or may not be included for core re
> urces like:
